...
serve(async (req) => {
  ...
  try {
    ...
    for (const notification of notifications) {
      ...
      console.log(`📱 Found ${tokens.length} device tokens (iOS: ${tokens.filter(t => t.platform === 'ios').length}, Android: ${tokens.filter(t => t.platform === 'android').length})`)
      ...
      for (const deviceToken of tokens) {
        try {
          ...
          if (response.ok) {
            console.log(`✅ Sent successfully to ${deviceToken.platform} device (member ${deviceToken.member_id})`)
            sentCount++
          } else {
            console.error(`❌ Failed to send to ${deviceToken.platform} (member ${deviceToken.member_id}):`, JSON.stringify(result))
            
            // Remover tokens inválidos
            if (result.error?.code === 404 || result.error?.status === 'NOT_FOUND' || 
                result.error?.status === 'INVALID_ARGUMENT' || result.error?.code === 'UNREGISTERED') {
    ...
    console.log(`\n📊 Summary: ${sentCount} notifications sent successfully`)
