...
async function sendAPNsNotification(token: string, title: string, body: string, data: any) {
  ...
  const apnsBundleId = Deno.env.get('APNS_BUNDLE_ID') || 'com.igreja.admin'
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📱 Found ${tokens.length} device tokens (iOS: ${iosTokens.length}, Android: ${androidTokens.length})`)

    let sentCount = 0
    const dataPayload = data || { type: type || 'aviso', aviso_id }

    // Enviar para iOS via APNs
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (response.status === 410 || response.status === 400) {
            console.log(`🗑️  Removing invalid iOS token for member ${deviceToken.member_id}`)
            await supabaseClient.from('device_tokens').delete().eq('id', deviceToken.id)
          }
        }
      } catch (error) {
        console.error(`❌ Error sending to iOS device ${deviceToken.id}:`, error.message)
      }
    }

    // Enviar para Android via FCM
      ...
      if (!serviceAccountJson) {
        ...
        for (const deviceToken of androidTokens) {
          try {
            ...
            if (response.ok) {
              console.log(`✅ Sent successfully to Android device (member ${deviceToken.member_id})`)
              sentCount++
            } else {
              console.error(`❌ Failed to send to Android (member ${deviceToken.member_id}):`, JSON.stringify(result))
              
              if (result.error?.code === 404 || result.error?.status === 'NOT_FOUND') {
                console.log(`🗑️  Removing invalid Android token for member ${deviceToken.member_id}`)
                await supabaseClient.from('device_tokens').delete().eq('id', deviceToken.id)
              }
            }
          } catch (error) {
            console.error(`❌ Error sending to Android device ${deviceToken.id}:`, error.message)
          }
        }
      }
    }

    console.log(`\n📊 Summary: ${sentCount} notifications sent successfully (${iosTokens.length} iOS, ${androidTokens.length} Android)`)
